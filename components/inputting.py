...
def blink():
    ...
    draw_input()
    gv.root.after(500, lambda: blink())

def draw_speech(event, canvas_color, is_empty_warning=False):
    x1 = 0
    y1 = 0
    x2 = event.width
    y2 = event.height
    ...
    if is_empty_warning:
        gv.canvas.itemconfig('speech', fill=color_converter(canvas_color))
    else:
        gv.canvas.config(bg=gv.color['root'])
        
        if not gv.canvas.find_withtag('speech'):
            gv.canvas.create_polygon(points, fill=color_converter(canvas_color), smooth=True, tags=['speech'], width=0)
        else:
            gv.canvas.coords('speech', *points)
def on_click(event):
    ...
    if input_text or selected_text:
        try:
            if is_shift:
                ...
                select_side = ''
                select_middle = -1
            index = gv.canvas.index('probe', f"@{event.x},{H}")
...
def on_drag(event):
    ...
    if input_text or selected_text:
        try:
            ...
            pass
        draw_input(gv.root, gv.canvas, gv.ui_font)
    else:
        cursor_index = 0
...
def draw_input():
    ...
    gv.canvas.tag_lower('speech')
    W = gv.canvas.winfo_width()
    H = gv.canvas.winfo_height()
    font_size = max(12, gv.root.winfo_width()//60,  gv.root.winfo_height()//20)
    if font == None or font.actual()['size'] != font_size: font = ft.Font(family=gv.ui_font, size=font_size)
    ...
    if not gv.canvas.find_withtag('text'):
        gv.canvas.create_rectangle(
            x2, H-H//4, x3, H//4,
            width = 0,
            fill = '#0000ff',
            tags = ['select']
        )
        gv.canvas.create_text(
            x1, H//2,
            text = input_text_1,
            font = font,
            fill = '#000000',
        ...
        gv.canvas.coords('select', x2, H-H//4, x3, H//4)
        
        gv.canvas.itemconfig('text1', text=input_text_1, font=font)
        gv.canvas.coords('text1', x1, H//2)
        
        gv.canvas.itemconfig('cursor', text='|', font=font, fill='#000000' if not color_flip else '#f7edc6')
        gv.canvas.coords('cursor', x_cursor, H//2)
        
        gv.canvas.itemconfig('text_select', text=selected_text, font=font)
        gv.canvas.coords('text_select', x2, H//2,)
        
        gv.canvas.itemconfig('text2', text=input_text_2, font=font)
        gv.canvas.coords('text2', x3, H//2)
        
        gv.canvas.itemconfig('probe', text=input_text[:cursor_index]+input_text[cursor_index:], font=font)
        gv.canvas.coords('probe', x1, H//2)
    if gv.canvas.find_withtag('speech'):
        if select_side == '':
            gv.canvas.tag_lower('select', 'speech')
        else:
            gv.canvas.tag_raise('select', 'speech')
...
def on_key(event):
    ...
    match event.keysym:
        ...
        case 'v':
            # Ctrl
            if is_ctrl:
                input_char(str(gv.root.clipboard_get()).replace('\n', ' '))
                cursor_index += len(str(gv.root.clipboard_get()).replace('\n', ' '))
            # Normal
            else:
